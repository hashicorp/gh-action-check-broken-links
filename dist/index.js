...
const FormattingElementList = __webpack_require__(153);
...
const doctype = __webpack_require__(334);
const foreignContent = __webpack_require__(427);
const ERR = __webpack_require__(665);
const unicode = __webpack_require__(56);
const HTML = __webpack_require__(141);
...
var word = __webpack_require__(122)
var whitespace = __webpack_require__(578)
...
var visitParents = __webpack_require__(162)
...
var whitespace = __webpack_require__(578)
var normalize = __webpack_require__(292)
...
const ErrorReportingTokenizerMixin = __webpack_require__(614);
...
var interrupt = __webpack_require__(418)
...
var removeIndent = __webpack_require__(774)
var interrupt = __webpack_require__(418)
...
async function run({ GITHUB_WORKSPACE }) {
    try {
        ...
        const whitelist = core
            ...
            .split(/\r?\n/)
            .filter(Boolean);
        const filesWithLinks = lib_1.getLinkInfoFromFiles(GITHUB_WORKSPACE, files);
        if (!filesWithLinks.length)
            return;
        const brokenLinks = await lib_1.collectBrokenLinks(baseUrl, filesWithLinks, whitelist);
        if (!brokenLinks.length)
            return;
        const annotations = lib_1.createAnnotations(brokenLinks);
...
var schema = __webpack_require__(844)
var caseSensitive = __webpack_require__(358)
...
var xtend = __webpack_require__(940)
var errors = __webpack_require__(881)
...
const { DOCUMENT_MODE } = __webpack_require__(141);
...
var visit = __webpack_require__(760)
var iteratee = __webpack_require__(291)
...
var decodeEntity = __webpack_require__(58)
...
Object.defineProperty(exports, "__esModule", { value: true });
exports.findAllLinks = exports.isAnchorLinkPresent = void 0;
const fs_1 = __importDefault(__webpack_require__(747));
const rehype_parse_1 = __importDefault(__webpack_require__(306));
const remark_parse_1 = __importDefault(__webpack_require__(923));
const unified_1 = __importDefault(__webpack_require__(536));
const unist_util_visit_1 = __importDefault(__webpack_require__(83));
const unist_util_find_1 = __importDefault(__webpack_require__(329));
function isAnchorLinkPresent(hash, markup) {
    // Remove '#' from hash to create anchor link
    const anchor = hash.slice(1);
    const ast = unified_1.default()
        .use(rehype_parse_1.default)
        .parse(markup);
    const match = unist_util_find_1.default(ast, (node) => {
        var _a, _b, _c;
        return (
        // <a name={anchor}></a>
        (node.tagName === 'a' &&
            ((_a = node.properties) === null || _a === void 0 ? void 0 : _a.name) === anchor) ||
            // <a href={hash}></a>
            (node.tagName === 'a' &&
                ((_b = node.properties) === null || _b === void 0 ? void 0 : _b.href) === hash) ||
            // <{el} id={anchor}></{el}>
            (node.type === 'element' &&
                ((_c = node.properties) === null || _c === void 0 ? void 0 : _c.id) === anchor));
    });
    return Boolean(match);
}
exports.isAnchorLinkPresent = isAnchorLinkPresent;
function findAllLinks(filepath) {
    const links = [];
    const ast = unified_1.default()
        .use(remark_parse_1.default)
        .parse(fs_1.default.readFileSync(filepath));
    unist_util_visit_1.default(ast, 'link', (link) => {
        links.push({
            url: link.url,
            ...(link.position
...
function getLinkInfoFromFiles(workspace, files) {
    return files
        .filter(filename => {
        return (filename.split('/').includes('pages') ||
            filename.split('/').includes('content'));
    })
        .map(filename => {
        const filepath = path_1.default.join(workspace, filename);
        const links = utils_1.findAllLinks(filepath);
        return {
            filename,
            links
        };
...
exports.getLinkInfoFromFiles = getLinkInfoFromFiles;
async function collectBrokenLinks(baseUrl, linkInfo, whitelist) {
    const errors = [];
...
exports.collectBrokenLinks = collectBrokenLinks;
function resolveUrl(baseUrl, urlPath, href) {
    const parsedHref = url_1.default.parse(href, false, true);
...
async function fetchStatusCode(href, retries = 5, retryDelay = 500) {
    ...
    try {
        ...
        if (hash && !hash.startsWith('#!')) {
            return utils_1.isAnchorLinkPresent(hash, await res.text()) ? statusCode : 502;
        }
        return statusCode;
    }
    catch (err) {
...
exports.fetchStatusCode = fetchStatusCode;
function createAnnotations(brokenLinks) {
...
var xtend = __webpack_require__(940)
var removePosition = __webpack_require__(318)
...
var xtend = __webpack_require__(940)
var entities = __webpack_require__(432)
...
var convert = __webpack_require__(457)
...
// Expose core.
proto.setOptions = __webpack_require__(94)
proto.parse = __webpack_require__(734)

// Expose `defaults`.
proto.options = __webpack_require__(551)
...
proto.blockTokenizers = {
  blankLine: __webpack_require__(372),
